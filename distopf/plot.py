...
def plot_voltages_timeseries(v: pd.DataFrame = None) -> go.Figure:
    """
    Parameters
    ----------
    v : pd.DataFrame, Dataframe containing solved voltages for each bus.
    ...
    if "name" not in v.columns:
        v["name"] = v["id"]
    v = v.melt(
        ignore_index=False, id_vars=["t", "id", "name"], var_name="phase", value_name="v"
    )
    v["v"] = v["v"].astype(float)
    fig = px.line(v, x="t", y="v", facet_col="phase", color="name")
    fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1].upper()))
    fig.for_each_xaxis(lambda a: a.update(title="Bus Name"))
    return fig
...
def plot_power_flows(s: pd.DataFrame) -> go.Figure:
    ...
    if "from_name" not in s.columns:
        s["from_name"] = s.fb
    if "to_name" not in s.columns:
        s["to_name"] = s.tb
...
def plot_power_flows_timeseries(s: pd.DataFrame) -> go.Figure:
    """
    Plot the active and reactive power flowing into each bus on each phase.
    Parameters
    ----------
    ...
    s = s.melt(
        ignore_index=True,
        id_vars=["t", "fb", "tb", "from_name", "to_name"],
        var_name="phase",
        value_name="s",
    )
    s["p"] = s.s.apply(lambda x: x.real)
    ...
    s = s.melt(
        ignore_index=True,
        id_vars=["t", "fb", "tb", "from_name", "to_name", "phase"],
        var_name="part",
        value_name="power",
    )
    fig = px.bar(
...
def plot_gens(p_gens: pd.DataFrame, q_gens: pd.DataFrame) -> go.Figure:
    ...
    fig : Plotly figure object
    """
    if "id" not in p_gens.columns:
        p_gens["id"] = p_gens.index
    if "name" not in p_gens.columns:
        p_gens["name"] = p_gens["id"]
    if "id" not in q_gens.columns:
        q_gens["id"] = q_gens.index
    if "name" not in q_gens.columns:
        q_gens["name"] = q_gens["id"]
...
def plot_device_timeseries(df: pd.DataFrame) -> go.Figure:
    """
    Plot the active and reactive power flowing into each bus on each phase.
    Parameters
    ----------
    df : pd.DataFrame
        Dataframe containing solved voltages for each bus.
        Typically generated by the LinDistModel.get_voltages() method.
    ...
    if "name" not in df.columns:
        df["name"] = df["id"]
    df = df.melt(
        ignore_index=False, id_vars=["t", "id", "name"], var_name="phase", value_name="value"
    )
    df["value"] = df["value"].astype(float)
    fig = px.line(df, x="t", y="value", facet_col="phase", color="name")
    fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1].upper()))
    fig.for_each_xaxis(lambda a: a.update(title="Time step"))
    return fig


def plot_gens_timeseries(p_gens: pd.DataFrame, q_gens: pd.DataFrame) -> go.Figure:
    """
    Plot the active and reactive power flowing into each bus on each phase.
    Parameters
    ----------
    ...
    gens = pd.merge(p_gens, q_gens, how="outer", on=["t", "id", "name", "phase"])
    gens.id = p_gens.id
    gens.name = p_gens.name
    gens.phase = p_gens.phase
    gens["P"] = p_gens["P"]
...
def plot_ders(ders: pd.DataFrame) -> go.Figure:
    ...
    fig : Plotly figure object
    """
    if "id" not in ders.columns:
        ders["id"] = ders.index
    if "name" not in ders.columns:
        ders["name"] = ders["id"]
    dec_var = ders.melt(
        ignore_index=False,
        var_name="phase",
