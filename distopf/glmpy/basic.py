...
class Gridlabd:
    ...
    def swing_nodes(self):
        return self.find_objects_with_property_value(
            "bustype", "SWING", search_types=["meter", "node"], prepend_class=False
        )
            ...
            if obj_type in self.model.keys():
                for obj_name in self.model.get(obj_type):
                    if self.model[obj_type][obj_name].get(obj_property) == value:
                        if prepend_class:
                            obj_list.append(
                                obj_type.strip("'").strip('"')
                                + ":"
                                + obj_name.strip("'").strip('"')
                            )
                        else:
    ...
    def get_object_type(self, obj_name: str, search_types: list = None):
        ...
        ):  # support receiving obj_name in the form class:obj_name e.g. "meter:node_2"
    ...
    def get_object_property_value(
        self, obj_name: str, obj_property: str, search_types: list = None
    ):
        """
    ...
    def run(self, tmp_model_path=None, file_names_to_read=None):
        ...
        if tmp_model_path is None:
            if self.base_dir_path is None:
                raise RuntimeError(
                    "No path is provided to add temporary directory to. Either provide, tmp_model_path "
        ...
        if self.model.get("player") is not None:
            ...
            for player_name in self.model["player"].keys():
                if self.model["player"][player_name].get("file") is not None:
                    old_path_name = (
                        tmp_model_path
                        / Path(self.model["player"][player_name].get("file"))
                    ).absolute()
                    shutil.copy(old_path_name, player_dir / old_path_name.name)
                    self.model["player"][player_name]["file"] = str(
                        Path("players/" + old_path_name.name)
                    )
    ...
    def total_load(self):
        ...
        delta_to_wye = (
            ...
            / np.sqrt(3)
            * np.array(
                [[1, 0, -1 * minus120], [-1 * minus120, 1, 0], [0, -1 * minus120, 1]]
            )
        )
        ...
        for load_name, load in self.model["load"].items():
            ...
            for i, ph in enumerate("ABC"):
                ...
                s_wye[i] += v[i] * np.conjugate(
                    complex(load.get(f"constant_current_{ph}", complex(0)))
                )
                s_wye[i] += v[i] * np.conjugate(
                    complex(load.get(f"constant_current_{ph}N", complex(0)))
                )
                # Constant impedance WYE connected load
                if f"constant_impedance_{ph}" in load.keys():
                    s_wye[i] += (v[i]) ** 2 / complex(
                        load.get(f"constant_impedance_{ph}", complex(0))
                    )
                if f"constant_impedance_{ph}N" in load.keys():
                    s_wye[i] += (v[i]) ** 2 / complex(
                        load.get(f"constant_impedance_{ph}N", complex(0))
                    )
                # ZIP Loads
            ...
            for i, ph in enumerate(["AB", "BC", "CA"]):
                ...
                # Constant current WYE connected load:
                s_delta[i] += v_delta[i] * np.conjugate(
                    complex(load.get(f"constant_current_{ph}", complex(0)))
                )
                # Constant impedance WYE connected load
                if f"constant_impedance_{ph}" in load.keys():
                    s_delta[i] += (v_delta[i]) ** 2 / complex(
                        load.get(f"constant_impedance_{ph}", complex(1))
                    )
            # print(f'{load_name}:')
            s_wye_tot += s_wye
            s_del_tot += s_delta
        ...
        for load_name, load in self.model["load"].items():
            ...
            if "D" not in load.get("phases"):
                for i, ph in enumerate("ABC"):
                    ...
                    # Constant impedance WYE connected load
                    if f"constant_impedance_{ph}" in load.keys():
                        s_wye[i] += np.abs(v[i]) ** 2 / np.conjugate(
                            complex(load.get(f"constant_impedance_{ph}", complex(0)))
                        )
                    if f"constant_impedance_{ph}N" in load.keys():
                        s_wye[i] += np.abs(v[i]) ** 2 / np.conjugate(
                            complex(load.get(f"constant_impedance_{ph}N", complex(0)))
                        )
                    # ZIP Loads
                    if f"base_power_{ph}" in load.keys():
                        base_power = float(load.get(f"base_power_{ph}"))
                        ...
                        impedance_fraction = float(
                            load.get(f"impedance_fraction_{ph}", 0)
                        )
                        p = base_power * (
                            power_fraction * power_pf
                            + current_fraction * current_pf
    ...
    def rename_object(self, obj_name: str, new_obj_name: str, obj_type: str = None):
        ...
        children = self.find_objects_with_property_value(
            "parent", obj_name, prepend_class=True
        )
        for child in children:
            self.get(child)["parent"] = new_obj_name
        if obj_type in _node_types:
            upstream_links = self.find_objects_with_property_value(
                "to", obj_name, search_types=_link_types, prepend_class=True
            )
            for upstream_link in upstream_links:
                self.get(upstream_link)["to"] = new_obj_name
            downstream_links = self.find_objects_with_property_value(
                "from", obj_name, search_types=_link_types, prepend_class=True
            )
            for downstream_link in downstream_links:
                self.get(downstream_link)["from"] = new_obj_name

        configurations = self.find_objects_with_property_value(
            "configuration", obj_name, prepend_class=True
        )
        for configuration in configurations:
            self.get(configuration)["configuration"] = new_obj_name
        conductor_A_refs = self.find_objects_with_property_value(
            "conductor_A",
            obj_name,
        ...
        conductor_B_refs = self.find_objects_with_property_value(
            "conductor_B",
            obj_name,
            search_types=["line_configuration"],
            prepend_class=True,
        ...
        conductor_C_refs = self.find_objects_with_property_value(
            "conductor_C",
            obj_name,
            search_types=["line_configuration"],
            prepend_class=True,
        ...
        conductor_N_refs = self.find_objects_with_property_value(
            "conductor_N",
            obj_name,
            search_types=["line_configuration"],
            prepend_class=True,
        ...
        spacing_refs = self.find_objects_with_property_value(
            "spacing", obj_name, search_types=["line_configuration"], prepend_class=True
        )
            ...
            if self.model.get(link_type) is not None:
                for link_name in self.model[link_type].keys():
                    ...
                    if link_type in [
                        ...
                        if (
                            self.model[link_type][link_name].get("configuration")
                            is not None
                        ):
    ...
    def add_helics(self, federate_name, config_path):
        ...
        self.add_object(
            "helics_msg", federate_name, configure=Path(config_path).as_posix()
        )
    ...
    def rename_all_loads(self, prefix=None):
        ...
        for load in loads:
            ...
            if parent is None:
                warnings.warn(
                    "Load is not a child object. This method assumes loads have parent nodes."
                )
                return
    ...
    def read_csv(filepath, **kwargs):
        ...
        """
        try:
            df = pd.read_csv(filepath, sep=",", header=1, index_col=0, **kwargs)
        except pd.errors.ParserError:
            df = pd.read_csv(filepath, sep=",", header=8, index_col=0, **kwargs)
        return df
