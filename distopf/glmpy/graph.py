...
def fix_reversed_links(g, source):
    ...
    if source in g:
        ...
        edge_attr = [
            g.get_edge_data(edge[0], edge[1]) or g.get_edge_data(edge[1], edge[0])
        ...
        for edge in g.edges:
            if edge not in digraph.edges and reverse_edge(edge) not in digraph.edges:
                missing_edges.append(
                    (edge[0], edge[1], g.get_edge_data(edge[0], edge[1]))
                )
        add_edges = []
        for edge in missing_edges:
            ...
            if d0 < d1:
                add_edges.append(
                    (edge[0], edge[1], g.get_edge_data(edge[0], edge[1])[0])
                )
            else:
                add_edges.append(
                    reverse_edge(
    ...
    if "sectionalizer" in model.keys():
        ...
        for sec in model["sectionalizer"]:
            for key in model["sectionalizer"][sec]:
                if (
                    "state" in key
                ):  # This could break if a secionalizer has open and closed phases
...
def draw_feeders(model: dict, feeder_swing_nodes: list = None, **options):
    ...
    for set_ in comp:
        ...
        edge_color = [
            g_.get_edge_data(edge[0], edge[1])[0]["color"] for edge in g_.edges
        ]
